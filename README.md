...
| 5   | [React](#React)                                                                                                 |
| 6   | [Next JS](#Next-JS)                                                                                                           |
| 7   | [Redux Toolkit And Redux](#Redux-Toolkit-And-Redux)                                                                                                                                     |
...
32. Расскажи про Рендеринг WEB-страницы
    ...
    3. CSSOM строится по мере парсинга CSS, но она не может быть использована для построения Render Tree,
    потому что может оказаться так, что следующий набор правил, может сделать какой-нибудь из узлов
    дерева невидимым на экране, это может вызвать перерасчёт стилей
    4. Когда DOM & CSSOM готовы, браузер создаёт Render tree, это дерево которое даёт
     представление о том, что в итоге будет отображаться на странице.
     В Render tree попадают только видимые элементы.
    5. Layout - этот этап определяет, где и как на странице, будут спозиционированы элементы и каковы будут связи между ними.
    ...
    7. Composite - разделение содержимого страницы на «слои», которые браузер будет перерисовывать.
    ...
    1)Relayout (или Reflow) — это процесс пересчета геометрии (положения и размеров) элементов в документе.
    Изменение любого свойства, влияющего на геометрию элемента или его соседей, вызовет рефлоу.
    ...
    2) Repaint — это процесс перерисовки пикселей на экране, когда внешний вид элемента изменился,
    но его геометрия (и геометрия других элементов) осталась прежней.
    ...
    3) Compositing (композитинг) — это финальный этап, когда браузер рисует
    предварительно подготовленные слои (layers) на GPU. Изменение свойств, которые обрабатываются композитором,
    позволяет избежать Relayout и Repaint, что наиболее эффективно для анимаций.
...
3. Строгий режим (strict mode)?
    ...
    1️⃣ Запрещает неявные глобальные переменные
    "use strict";
    x = 10; // ❌ ReferenceError
    2️⃣ Меняет this
    В функциях this === undefined, а не window
    3️⃣ Запрещает дубликаты параметров
    "use strict";
    function f(a, a) {} // ❌ SyntaxError
    4️⃣ Запрещает небезопасные конструкции
    with
    присваивание NaN, undefined
    удаление переменных
...
4. Разница между function declaration и function expression?
    ...
    FD - хостится/поднимается - создаётся интерпретитором до выполнения кода, следовательно можно вызвать до 
    объявления и это не вызовет ошибку.
    ```
    
5. Что такое new Function expression? Как объявить функцию с помощью этого выражения?
    ```jsx
    new Function expression — это способ динамически создать функцию из строки кода во время выполнения программы в JavaScript.
...
8. Что такое Temporal Dead Zone (TDZ) и когда она возникает?
    
    ```jsx
    Temporal Dead Zone (TDZ) — это промежуток времени
    между входом в область видимости и фактическим объявлением переменной,
    когда переменная уже существует (зарезервирована), но к ней нельзя обратиться
    ```
...
10. Разница между let, var, const
...
16. Что такое функция-генератор? Чем она отличается от остальных функций?

    ```jsx
    Функция-генератор — это функция, которая может приостанавливать свое выполнение и возобновлять его позже.
    Вместо того чтобы возвращать одно значение и завершаться, генератор может выдавать (генерировать)
    последовательность значений по одному за раз, сохраняя свое состояние между вызовами.
    yield - вместо return; При вызове функции-генератора тело функции не выполняется.
    Вместо этого она возвращает специальный объект — итератор
    ...
    const generatorCounter = counter();
    
    generatorCounter.next().value; // 1
    generatorCounter.next().value; // 2
    generatorCounter.next().value; // 3
    ```

17. Что такое итератор в рамках JS? Как сделать объект итерируемым?
    Что делает метод `next()` и что он возвращает? Как работает цикл `for...of` с итерируемыми объектами?
    Как связаны генераторы и итераторы в JavaScript?

     ```jsx
    1. Итератор — это объект, который предоставляет механизм последовательного доступа к элементам коллекции или последовательности.
    2. Чтобы объект стал итерируемым (то есть чтобы его можно было обойти в цикле for...of), он должен реализовать Symbol.iterator метод. Этот метод должен возвращать итератор.
    3. Метод next() — это сердце итератора. Его задача — вернуть следующий доступный элемент в последовательности.
    {
      value: любойТипДанных, // текущее значение (если done: false)
      done: boolean          // true - если элементов больше нет
    }
    4. Цикл for...of — это синтаксический сахар, который скрывает всю работу с итераторами. Под капотом он по циклу while пробегается по итерируемому объекту, пока done !== true
    5. Генераторы — это фабрики итераторов. Это самый простой способ создать итератор.
    Генератор возвращает итератор: Когда ты вызываешь функцию-генератор, она возвращает объект-генератор, который одновременно является итератором (у него есть метод next())
...
18.    .map, .filter, .reduce, .forEach для чего нужны, особенности использования
...
18. Отличие стрелочных функций от функций объявленных через function
...
19. Что такое самовызывающиеся функции (IIFE)? Зачем их использовали до выхода переменных let const?
      ```jsx
    IIFE - функция, которая описывается и сразу же вызывается
    (function() {
      console.log('Выполняюсь сразу!');
    })();
    До появления let и const в ES6 (2015) в JavaScript было только var.
    У var была только две области видимости: глобальная и функциональная. Не было блочной области видимости.
    IIFE создавали изоляцию кода, создавая новую функциональную область видимости,
    что давало инкапсуляцию, приватные переменные
    ```


20. Что такое замыкание
...
22. Что такое прототипное наследование? Как создать объект без прототипа в JavaScript?
...
23. Что такое Set и Map и для чего они нужны?
...
24. как определить наличие свойств в объекте
    
    ```jsx
    Первый способ: вызов функции obj.hasOwnProperty(property) где параметром передаётся само свойство,
    и в случае если оно в объекте есть, возвращается true - при этом не заглядывает
    в прототипы
    upd: Новое Object.hasOwn(obj, property)
    Второй способ: воспользоваться оператором in ("key" in obj), который тоже вернёт
    true, при этом заглядывает в прототипы
    ```
    
25. Что такое дискрипторы свойств в JS?
    ```jsx
    1.Дескрипторы свойств — это объекты, которые описывают поведение и характеристики конкретного свойства объекта. Они определяют, можно ли свойство изменять, удалять, будет ли оно видно при перечислении и т.д.

    Каждое свойство объекта имеет три специальных флага (дескриптора) и значение:
    writable — возможность изменения значения
    enumerable — видимость при перечислении
    configurable — возможность изменения дескрипторов и удаления свойства
    value — непосредственно значение свойства
    2. writable контролирует, можно ли изменять значение свойства через оператор присваивания (=).
    3. enumerable определяет, будет ли свойство появляться при перечислении (цикл for...in, Object.keys() и т.д.). 
    4. configurable контролирует две вещи:
    Можно ли удалять свойство через delete
    Можно ли изменять дескрипторы свойства (кроме writable с true на false)
    5. Object.seal() делает два изменения для всех свойств объекта:
    Устанавливает configurable: false для всех существующих свойств
    Запрещает добавление новых свойств (но это не дескриптор, а внутренний механизм) 
    6. Object.freeze() делает все то же, что и seal(), плюс запрещает изменение значений:
    Устанавливает configurable: false для всех свойств
    Устанавливает writable: false для всех свойств данных (не геттеров/сеттеров)
    Запрещает добавление новых свойств
    
    seal и freeze работают поверхностно, не затрагивая вложенные объекты
    ```




26. Что такое геттер и сеттер в объекте JavaScript?
    ```jsx
    Геттеры (getters) и сеттеры (setters) — это специальные функции, которые позволяют определять поведение
    при чтении и записи свойства объекта. Они выглядят как обычные свойства, но внутри работают как методы.
    Геттер — вызывается при чтении значения свойства
    Сеттер — вызывается при присвоении значения свойству
    const user = {
      firstName: 'John',
      lastName: 'Doe',
    ...
    console.log(user.fullName); // "John Doe" (вызвался геттер)
    user.fullName = 'Jane Smith'; // (вызвался сеттер)
    console.log(user.firstName); // "Jane"
    console.log(user.lastName); // "Smith"
    ```

27.  Какие способы создание объекта существуют
...
28. Какие значения будут являться false значениями
    
    ```jsx
    null, undefined, NaN, 0, "", false, BigInt(0)
    ```
    
29. Разница между синхронными и асинхронными функциями
...
33. **Разница между Promise.all(), Promise.allSettled(), Promise.any() и Promise.race()?**
...
34. Как использовать async/await для ассинхронных запросов
...
35. **Для чего используется цикл for…of?**
...
36.  Что такое spread-оператор?
    
    ```jsx
    ... - спред оператор, используется с объектами (включая массивы),  разворачивает объект,
    часто используется для копирования (только первого уровня объекта) или раскрытия внутреннего массива во внешний
    a = [1, 2]
    b = [...a, 3, 4] //[1, 2, 3, 4]
    ```
    
35. Что такое rest-оператор?

    ```jsx
    ...rest оператор, часто используется для деструктуризации объекта или массива, 
    также можно использовать в параметрах функции для принятия произвольного количества параметров
...
36. Как избежать ссылочной зависимости при копировании объекта
...
37. Как поменять контекст функции
...
48. **Разница между cookie, sessionStorage и localStorage**
...
49. Что такое и зачем нужен requestAnimationFrame?
    ```jsx
    Это метод браузера, который планирует выполнение функции перед ближайшей перерисовкой экрана,
    используется для создания плавных анимаций и эффективных визуальных обновлений.
    Он синхронизируется с частотой обновления монитора (обычно 60 Гц) и приостанавливается в неактивных вкладках,
    в то время как таймеры выполняются независимо от перерисовок, что может вызывать рывки или излишнюю нагрузку.

    Внутри callback-функции нужно обновлять состояние анимации, выполнять необходимые изменения в DOM,
    а затем рекурсивно вызывать requestAnimationFrame для продолжения анимации.
    Создастся бесконечный цикл анимации, где каждый кадр будет планировать следующий,
    что идеально для непрерывной анимации, но требует явного условия остановки.
    ```
...
58. **Что такое Полифил**
    ...
    ```jsx
        Полифил (Polyfill) — это код (обычно JavaScript), который добавляет современную
        функциональность в старые браузеры, которые её не поддерживают нативно.
        Что делает полифил:
...
59. Что такое лези-лоадинг? Зачем он нужен?
    ```jsx
        Lazy-loading — это техника отложенной загрузки ресурсов (модулей, компонентов, изображений),
        которые не нужны при первоначальной загрузке страницы. Он экономит ресурсы и ускоряет время первой отрисовки
...
60. Что такое Tree-shaking? Нужно ли его настраивать в современных бандлерах? От чего зависит, будет ли три-шейкаться библиотека?
    ```jsx
        Tree-shaking (встряхивание дерева) — это техника оптимизации, которая удаляет из финальной сборки
        "мертвый код" (dead code) — неиспользуемые экспорты, функции, модули.
        От чего зависит, будет ли три-шейкаться библиотека?
        1) ES Modules поддерживает tree-shaking из коробки (не нужно настраивать), CommonJS нет.
        2) Side effects
        3) Структуры импортов
        4) наличия .mjs файла
    ```

61.  Что такое сборщик мусора (garbage collector) в JS и зачем он нужен? Когда вызывается?
    ```jsx
        Сборщик мусора - автоматический процесс освобождения памяти от объектов,
        на которые нет ссылок, чтобы предотвратить утечки.
        Он считает кол-во ссылок на объект из корня и помечает (mark), если 0 ссылок,
        объект удаляется (sweep)
        Современные движки (mark-and-sweep) находят циклические ссылки
        (объектА → объектБ → объектА), так как они недостижимы из корня.
        GC вызывается когда движку не хватает памяти или в периоды простоя (алгоритм недетерминирован).
    ```

63. Что такое Web Components и зачем они нужны?

    ```jsx
        Web Components — это набор стандартов W3C, позволяющих создавать переиспользуемые
        кастомные HTML-элементы с инкапсулированной логикой и стилями.
        
        Плюсы: Нативные, поддерживаемые, нулевой размер бандла, работают в любом фреймворке
        Минусы: Более низкоуровневые, нет управления состоянием и реактивности,
        меньше фишек по сравнению с фреймворками, нет SSR
    ```


64.  Что такое Shadow DOM и зачем он используется?
    ```jsx
        Shadow DOM — это часть спецификации Web Components, которая позволяет создавать изолированное DOM-дерево,
        прикреплённое к элементу, но скрытое от основного документа.
        Зачем используется:
        1) Изоляция — стили и скрипты внутри Shadow DOM не влияют на внешний документ и наоборот
        2) Инкапсуляция — скрывает внутреннюю реализацию компонента от внешнего мира
        3) Переиспользование — позволяет создавать самодостаточные компоненты без конфликтов
        4) Стандартизация — используется в нативных элементах браузера (например, video имеет внутренний Shadow DOM)
    ```

65. Что такое slot и как он используется для проекции контента в Shadow DOM?
    ```jsx
    <slot> — это элемент-заполнитель внутри Shadow DOM, который определяет места,
    куда будет проецироваться контент из светлого DOM (light DOM).
    ...
    shadowRoot.innerHTML = `
      ...
      <div class="card">
        <h2><slot name="name">Имя по умолчанию</slot></h2>
        <p><slot name="email">Email не указан</slot></p>
...
8. Что представляет из себя WebSocket? Опиши полный жизненный цикл WebSocket-соединнения.
Как передать авторизацию в WebScoket?

```jsx
    WebSocket — это протокол двусторонней связи поверх TCP, обеспечивающий полнодуплексный (full-duplex) обмен данными между клиентом и сервером в реальном времени.
    В отличие от HTTP, который работает по модели запрос-ответ, WebSocket устанавливает постоянное
    соединение, позволяя обеим сторонам отправлять данные в любой момент без необходимости создания нового запроса.
    Жизненный цикл:
    1. Установка соединения (Handshake)
    Соединение начинается с HTTP-апгрейда, клиент запрашивает, сервер отвечает
    2. Открытое соединение (Open State)
    3. Обмен данными (Data Transfer)
    4. Поддержание соединения (Keep-alive)
    5. Закрытие соединения (Closing)
    6. Обработка ошибок

    Авторизацию можно передать в handshake: в квери (не безопасно для токенов), в http-загаловке или субпротоколе. Можно передать после установки соединения в сообщении.
    Если используется сессия, куки автоматически отправляются с handshake
```
9. Что такое Server Sent Events? Зачем он нужен?

```jsx

    Server-Sent Events (SSE) — это технология, позволяющая серверу отправлять данные клиенту через однонаправленное постоянное HTTP-соединение.
    В отличие от WebSocket, который обеспечивает двустороннюю связь, SSE предназначен исключительно для потоковой передачи данных от сервера к клиенту.
    Особенности:
    1. Однонаправленный поток данных (сервер → клиент)
    
    2. Работает поверх обычного HTTP/HTTPS
    
    3. Использует стандартный порт 80/443
    
    4. Протокол текстовый, основан на формате событий
```

10. Что такое Long Polling? Что такое Short Polling? Зачем они нужны, если есть WebSocket'ы?

```jsx
    Polling (опрос) — это техника, когда клиент регулярно отправляет запросы на сервер для проверки наличия новых данных.
    ShortPolling - запрашивание данных у сервера через равномерный промежуток времени (допустим в setInterval), каждый запрос завершается ответом, даже если данных нет
    LongPolling - запрашивание данных после каждого ответа сервера (ждем ответ и после сразу запрашиваем данные снова). Сервер приостанавливает ответ до появления данных или истечения таймаута.
    
    1. WSS Может быть избыточным решением
    2. Мобильные сети, брэндмауэры и прокси могут рвать не http-подключение
    3. Stateless архитектура, сервер не хранит состояние при polling
    4. polling легче масштабировать, любой запрос можно добавить, wss труднее
    5. polling кэшируется
    6. Можно использовать как фолбэк WSS

```

11. Что такое CSP? Зачем он нужен?

    ```jsx
        Content Security Policy (CSP) — это механизм безопасности, который помогает предотвращать XSS-атаки (Cross-Site Scripting)
        и другие атаки с внедрением контента, позволяя разработчикам контролировать,
        какие ресурсы могут загружаться и выполняться на странице.

        CSP работает по принципу "белого списка" — вы явно указываете,
        откуда можно загружать различные типы ресурсов (скрипты, стили, изображения и т.д.).
        Все остальные источники блокируются.
        Обычно сервер отдает страницу с CSP заголовком
        пример
        ```
        HTTP/1.1 200 OK
...
12.  Какие версии HTTP существуют? Перечисли все

    Что нового добавилось в HTTP/2? Там было два ключевых нововведения

    На основе какого низкоуровневого протокола реализован HTTP/2? А HTTP/3? Чем отличаются эти протоколы?

    ```jsx
        HTTP/0.9 (1991) – Первоначальная, крайне примитивная версия ("однострочный протокол").
    
        HTTP/1.0 (1996, RFC 1945) – Введение версий протокола, заголовков, статус-кодов, поддержка разных типов данных (MIME).
        
        HTTP/1.1 (1997, RFC 2068; актуальная - RFC 7230-7235, 2014) – Доминирующая версия на протяжении более 20 лет. Ключевые улучшения: постоянные соединения, pipelining, виртуальные хосты, кэширование.
        
        HTTP/2 (2015, RFC 7540) – Основополагающая модернизация для повышения производительности.
        
        HTTP/3 (2022, RFC 9114) – Новая версия, меняющая транспортный протокол с TCP на QUIC.
        
        Что нового добавилось в HTTP/2? Два ключевых нововведения:
        1. Мультиплексирование - несколько запрос/ответ в рамках одного соединения
        2. Бинарный протокол вместо текстого. Сообщения разбиваются на небольшие, управляемые фреймы (заголовочные, data, приоритеты и т.д.), которые затем собираются на другой стороне. Это делает обработку быстрее, компактнее и надежнее.
        
        HTTP/2 и ранее - TCP
        HTTP/3 - QUIC
        QUIC - Устойчивее, быстрее, всегда шифруется, устойчив к смене сети (TCP обрывается)

        Протокол определяется серваком, чекает может ли http/3, и фолбэкает вниз по списку
    ```
13. Что такое DNS?
    Распиши все этапы резолва ip по домену test.example.domain.ru
    Что такое A-запись в контексте DNS? Что такое NS запись?

    ```jsx
        DNS (Domain Name System) — это распределенная иерархическая система,
        которая преобразует понятные человеку доменные имена (например, google.com)
        в машинные IP-адреса (например, 142.250.185.46).

        Полный процесс резолва (разрешения) имени test.example.domain.ru:
        1) Проверка локального кэша и hosts-файла: Браузер/приложение запрашивает IP-адрес у ОС, ОС смотрим кэш hosts,
        отдает если есть, если нет запрос отправляется на DNS-резолвер (сервер провайдера или публичный)
        2) Рекурсивный резловер (идем по имени домена справа налево): спрашивает регион ru у корневого сервера,
        корневой говорит какие отвечают за ru, спрашиает у них Где зона domain.ru?
        Резолвер обращается к NS-серверам домена domain.ru: «Какой IP у test.example.domain.ru?»
        Они дают инфу о серверах которые это знают «За example.domain.ru отвечают ns1.example.domain.ru и ns2.example.domain.ru».
        Резолвер обращается к ним и они отдают ему IP
        3) Резолвер возвращает IP, комп кэширует и страница открывается в браузере
        
        A-запись (Address Record) — это фундаментальный тип записи DNS,
        который связывает доменное имя с IPv4-адресом (32-битным).
        test.example.domain.ru.  IN  A  192.0.2.100
        NS-запись (Name Server Record) — указывает, какие DNS-серверы являются авторитативными для данной зоны (домена).
        То есть, кому можно доверять в вопросе IP-адресов этого домена.
        domain.ru.  IN  NS  ns1.domain.ru.
        domain.ru.  IN  NS  ns2.domain.ru.
    ```

14. Что такое WebWorker и зачем он нужен?
    ```jsx
    Web Worker — это механизм в веб-браузерах, который позволяет выполнять JavaScript-код в фоновом потоке,
    отдельном от основного потока веб-страницы (главного потока, отвечающего за интерфейс).
    Главная цель — не блокировать пользовательский интерфейс (UI).
    post.message можно передовать простые типы данных, бинарные объекты, коллекции (обычные).
    if (window.Worker) {
      // Создаем новый воркер, указывая путь к его скрипту
      const myWorker = new Worker('worker.js');
     
      myWorker.postMessage('Привет, воркер!');
      myWorker.postMessage({ command: 'start', data: [1, 2, 3] }); // Можно отправлять объекты
      myWorker.onmessage = function(event) {
          console.log('Сообщение от воркера:', event.data);
          // event.data — это данные, присланные из воркера
    };
    
    // Обработка ошибок из воркера
        myWorker.onerror = function(error) {
          console.error('Ошибка в воркере:', error.message);
        };
      console.log('Воркер создан и запущен');
    } else {
