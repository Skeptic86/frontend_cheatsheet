...
31. Распишите, чем отличаются следующие параметры тега link
    ...
    ```html
        preload — грузит критический ресурс немедленно с высоким приоритетом (шрифт, скрипт для текущей страницы).
    
        prefetch — заранее кэширует ресурс для навигации по сайту (скрипт следующей страницы) с низким приоритетом.
        
        dns-prefetch — только резолвит DNS домена, с которого будут ресурсы, экономит ~20–120 мс.
        
        preconnect — резолвит DNS, устанавливает TCP и TLS для домена, с которого будут ресурсы (дороже, но полная подготовка соединения).
        
        prerender — не просто грузит, а полностью рендерит всю страницу в фоне (тяжело, почти не используется).
    ```
...
15. Единицы измерения(px и другие)
...
54. **Critical Rendering Path**
    ...
    1) DOM дерево html'а, создается параллельно
    2) CSSOM дерево стилей, создается синхронно, блокирует отрисовку.
    ```Стили, описанные в документе ниже, могут переопределять и изменять стили, определённые ранее. Так что если мы начнём использовать CSS-стили до того, как будет разобрана таблица стилей, мы можем столкнуться с ситуацией, когда стили будут применяться неверно.```
    3) JS по дефолту блочит парсер, если не async или defer
    4) Render-дерево — это совокупность DOM и CSSOM. Это дерево, которое даёт представление о том, что в конечном итоге будет отображено на странице. Это означает, что оно захватывает только видимый контент и не включает, например, элементы, которые были скрыты с помощью CSS-правила display: none.
    5) Раскладка — это то, что определяет размер видимой области документа (viewport), которая обеспечивает контекст для стилей CSS, зависимых от него, например, проценты или единицы вьюпорта.
    6) Наконец, на шаге отрисовки, видимый контент страницы может быть преобразован в пиксели, чтобы появиться на экране.
